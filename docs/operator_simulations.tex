...
\begin{figure}[htbp]
\centering
\makebox[\textwidth][c]{\includegraphics[width=1.18\textwidth]{figures/frame_homogeneous.png}}
...
\begin{figure}[htbp]
\centering
\makebox[\textwidth][c]{\includegraphics[width=1.18\textwidth]{figures/frame_layered.png}}
...
\begin{figure}[htbp]
\centering
\makebox[\textwidth][c]{\includegraphics[width=1.18\textwidth]{figures/fig_source_arm.png}}
...
\begin{figure}[htbp]
\centering
\makebox[\textwidth][c]{\includegraphics[width=1.16\textwidth]{figures/fig_ic_bc_residuals.png}}
...
\begin{figure}[htbp]
\centering
\makebox[\textwidth][c]{\includegraphics[width=1.16\textwidth]{figures/fig_physics_informed.png}}
...
\begin{figure}[htbp]
\centering
\makebox[\textwidth][c]{\includegraphics[width=1.18\textwidth]{figures/marmousi_dataset.png}}
\caption{The Marmousi arm. Top: one profile per contrast band, with the trace it
was drawn from and the depth of the window centre; right, the distribution of
within-sample heterogeneity against the synthetic sampler --- real geology varies
$4.1\times$ more violently along a column. Bottom: the FD reference fields, with
a synthetic sample for comparison; the clean V of the synthetic case fills with
reflections once the medium is real. Regenerate with
\texttt{wave/operator\_sim/figure\_marmousi\_dataset.py}, which recomputes every
number in the figure, including the caption's separation.}
\end{figure}
...
realised minimum train-to-validation separation is 324\,m (Marmousi), 400\,m
...
here.
\item \textbf{The forced arm's target is not grid-converged.} \S12.2 measures
9.72\,\% mean target error at \texttt{refine=1}, and the synthetic arm was
rebuilt at \texttt{refine=8} in response --- but the forced arm (\S10, \S11) was
not. \texttt{fd\_source.py} steps directly on the 64-point output grid and has
no refinement path, and \texttt{generate\_dataset\_source.py} has no
\texttt{-{}-refine} flag. Fixing it means adding refinement to the forced solver
--- interpolate $E$, $\rho$ and $s(x)$ up to a fine grid, solve, subsample ---
not a flag change. Every number in \S10 and \S11 therefore carries the caveat
\S12.2 raises for the original synthetic arm, and \S12.2's DeepONet result
suggests the ranking there could move.

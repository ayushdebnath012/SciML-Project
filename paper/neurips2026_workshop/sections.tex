...
Both second-order schemes stop improving at $\FDFloor\%$, and Table~\ref{tab:reffloor}
shows where: going from $N_x = 1000$ to $N_x = 8000$ --- a $64\times$ increase in
...
\input{tables/pinn_main}

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig_cost_accuracy}
\caption{What accuracy costs. Wall-clock seconds against relative $L^2$, both
logarithmic. Connected markers are the classical refinement ladders (CPU,
single core); hollow markers are the spectral solver agreeing with the
reference to its own precision rather than a measured error. Diamonds are
individual PINN runs at their measured training time on an H100. The dotted
line at $100\%$ is the score of predicting zero. The two families are separated
by roughly four orders of magnitude in time and two to three in error.}
\label{fig:cost}
\end{figure}
...
the spectral solver goes further still. Figure~\ref{fig:cost} places both
families on the same axes. Nothing about this is specific to a weak baseline:
...
Table~\ref{tab:pinnvariants} reports the SOAP optimiser, residual-based
attention (RBA) in place of causal weighting, and an L-BFGS-only schedule with
no first-order phase. The entry that matters here is RBA, because it is the one

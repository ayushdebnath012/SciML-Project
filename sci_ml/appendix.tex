...
Two tables referenced from the body. Neither carries the main claim, so both sit
here rather than against the page limit.
...
Table~\ref{tab:allruns} lists every run in the study --- \AllRunCount{} in
total, across the main sweep and the weighting ablation. The tables in the body
show slices chosen to make a point; this one hides nothing, so any cell of any
other table can be checked against it.

\input{tables/all_runs}
